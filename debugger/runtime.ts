...
import { FrameSourceLocation, SteppingResultType, TraceReplayEngine, TraceReplayStackFrame } from './traceReplayEngine'
...
export class AvmRuntime extends EventEmitter {
  ...
  public start(stopOnEntry: boolean, debug: boolean) {
    this.nextTickWithErrorReporting(() => {
      if (debug) {
        for (const [_, sourceDescriptor] of this.engine.programHashToSource.entriesHex()) {
          if (!sourceDescriptor) {
            continue
          }
    ...
    if (reverse) {
      ...
      switch (result.type) {
        ...
        case SteppingResultType.EXCEPTION:
          this.sendEvent(RuntimeEvents.stopOnException, result.exceptionInfo?.message)
          return false
      }
    } else {
  ...
  private findSourceDescriptorsForPath(filePath: string): Array<{ descriptor: ProgramSourceDescriptor; sourceIndex: number }> {
