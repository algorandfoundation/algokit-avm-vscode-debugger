...
import { AvmDebuggingAssets, ByteArrayMap, ProgramSourceDescriptor, ProgramSourceDescriptorRegistry } from './utils'
...
export class TraceReplayEngine {
  ...
  public programHashToSource: ByteArrayMap<ProgramSourceDescriptor | undefined> = new ByteArrayMap()
  ...
  public async loadResources(debugAssets: AvmDebuggingAssets) {
    ...
    for (let groupIndex = 0; groupIndex < simulateResponse.txnGroups.length; groupIndex++) {
      ...
      for (let txnIndex = 0; txnIndex < group.txnResults.length; txnIndex++) {
        this.setupTxnTrace(simulateResponse, programSourceDescriptorRegistry, groupIndex, txnIndex)
      }
    }
  ...
  private setStartingStack(simulateResponse: algosdk.modelsv2.SimulateResponse) {
  ...
  private resetCurrentAppState() {
    this.currentAppState = new Map(Array.from(this.initialAppState.entries(), ([key, value]) => [key, value.clone()]))
  }
  ...
  private setupTxnTrace(
    ...
    if (trace.logicSigTrace) {
      this.fetchProgramSourceInfo(programSourceDescriptorRegistry, trace.logicSigHash!)
    }
    visitAppTrace(txnPath, txn.txnResult, trace, (path, programHash, txnInfo, opcodes) => {
      this.fetchProgramSourceInfo(programSourceDescriptorRegistry, programHash)
  ...
  private fetchProgramSourceInfo(programSourceDescriptorRegistry: ProgramSourceDescriptorRegistry, programHash: Uint8Array) {
    if (this.programHashToSource.has(programHash)) {
      return
    }
    const sourceDescriptor = programSourceDescriptorRegistry.findByHash(programHash)
  ...
  public backward(): SteppingResult {
    ...
    do {
      ...
      if (this.stack.length === 0) {
        ...
        return exceptionInfo ? SteppingResult.exception(exceptionInfo) : SteppingResult.end()
      }
    } while (this.stack.length < length)
    return SteppingResult.ok()
  }
...
export abstract class TraceReplayStackFrame {
  ...
  public abstract backward(stack: TraceReplayStackFrame[]): ExceptionInfo | void
}
...
export class TopLevelTransactionGroupsFrame extends TraceReplayStackFrame {
  ...
  private frameForIndex(index: number): TransactionGroupStackFrame {
    ...
    const txnGroupFrame = new TransactionGroupStackFrame(this.engine, [index, 0], txnInfos, txnTraces, failureInfo)
...
export class TransactionGroupStackFrame extends TraceReplayStackFrame {
  ...
  public name(): string {
    return `${this.txnPath.length > 2 ? 'inner ' : ''}transaction ${this.txnIndex}`
  }

  public sourceFile(): FrameSource {
    return {
      name: `${this.txnPath.length > 2 ? 'inner-' : ''}transaction-group-${this.txnPath.slice(0, -1).join('-')}.json`,
  ...
  public forward(stack: TraceReplayStackFrame[]): ExceptionInfo | void {
    ...
    if (this.failureInfo && pathStartWith(this.failureInfo.path, this.txnPath.slice(1))) {
      if (this.failureInfo.path.length === this.txnPath.length - 1) {
        if (
          currentTxnTrace &&
          (currentTxnTrace.logicSigTrace || currentTxnTrace.approvalProgramTrace || currentTxnTrace.clearStateProgramTrace)
        ) {
          // Fail in the trace
    ...
    if (this.logicSigStatus === ProgramStatus.STARTING && currentTxnTrace) {
      const logicSigFrame = new ProgramStackFrame(
        ...
        this.appStatus === ProgramStatus.NOT_STARTED ? undefined : childFailureInfo,
      )
...
export class ProgramStackFrame extends TraceReplayStackFrame {
  ...
  public forward(stack: TraceReplayStackFrame[]): ExceptionInfo | void {
    ...
    if (!this.handledInnerTxns && spawnedInners && spawnedInners.length !== 0) {
      ...
      const innerGroupFrame = new TransactionGroupStackFrame(this.engine, expandedPath, innerGroupInfo, innerTraces, innerFailureInfo)
    ...
    } else if (this.failureInfo && pathsEqual(this.txnPath.slice(1), this.failureInfo.path)) {
